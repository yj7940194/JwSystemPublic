...
public interface TCourseMapper extends BaseMapper<TCourse> {
    ...
    @Select("SELECT c.`name`,tc.`id`,teacher.`tid`,w.time wname,s.`week` sw,s.section sse,cs.`name` csname,e.`name` ename,t.`name` tname,co.`name` collegeName,teacher.`tname` teacherName,c.`credit`,c.`total_time`,csys.`name` systemName,tc.`end` FROM t_course c,t_week w,t_section s,t_cstatus cs, t_examway e,t_team t,t_college co,t_teacher teacher,`teacher_course` tc,`course_system` csys WHERE tc.`cid` = c.`id` AND tc.week_id = w.id AND c.way_id = e.id AND tc.team_id = t.id AND tc.section_id = s.id AND c.cstatus_id = cs.id AND  c.`system_id` = csys.`id` AND teacher.`college_id` = co.`id` AND tc.`teacher_id` = teacher.tid AND tc.`status` = '1' AND tc.`end` = 1 AND teacher.`college_id` = #{collegeId} AND tc.`apply` = 1")
    IPage<CourseDTO> courseApply(Page page, @Param("collegeId") String collegeId);

    @Select("SELECT c.`name`,tc.`id`,teacher.`tid`,w.time wname,s.`week` sw,s.section sse,cs.`name` csname,e.`name` ename,t.`name` tname,co.`name` collegeName,teacher.`tname` teacherName,c.`credit`,c.`total_time`,csys.`name` systemName,tc.`end` FROM t_course c,t_week w,t_section s,t_cstatus cs, t_examway e,t_team t,t_college co,t_teacher teacher,`teacher_course` tc,`course_system` csys WHERE tc.`cid` = c.`id` AND tc.week_id = w.id AND c.way_id = e.id AND tc.team_id = t.id AND tc.section_id = s.id AND c.cstatus_id = cs.id AND  c.`system_id` = csys.`id` AND teacher.`college_id` = co.`id` AND tc.`teacher_id` = teacher.tid AND tc.`status` = '1' AND tc.`end` = 1 AND tc.`apply` = 1")
    IPage<CourseDTO> courseApplyAll(Page page);
